...
// Confidence model: deterministic heuristic.
  ...
  if (best) {
    ...
    // The specified cheese type (entities.cheese) is preserved for override application later.
    {
      ...
      if (best.id && _cheeseUpgradeMap[best.id] && _hasCheeseSignal) {
        const _upgradedId = _cheeseUpgradeMap[best.id];
    ...
    if (intent === "place_order") {
      if (!(state.order && state.order.pending_item && state.order.pending_item.exists)) {
        ...
        if (_parsedQty && _parsedQty >= 2) {
          // Fix Bug 4: guard against item names that start with ANY digit.
          // "two egg omelet" → qty=2, item="2 Egg Omelet" — the leading "2" is part of the item name.
          // "3 egg omelet" → qty=3, item="2 Egg Omelet" — still wrong; any digit-named item blocks external qty.
          const _itemNameLeadNum = /^\d+\s/.test(best?.raw?.name || "");
...
function dispatchEmit(state, emit, config) {
  ...
  switch (type) {
    ...
    case "reprice_pending_item": {
      ...
      const base = getBasePriceForFormat(mi, pi.format);
...
function applyPhaseInput(state, userText, config) {
  ...
  if (phase === "AWAITING_TOPPINGS_OR_CONDIMENTS") {
    ...
    if (/\bas\s+it\s+comes?\b/i.test(text) || /\bthe\s+usual\b/i.test(text) ||
        ...
        /\bjust\s+as\s+it\s+is\b/i.test(text) ||
        /\bas\s+it\s+is\b/i.test(text)) {
    ...
    // Must detect BEFORE hasAny so we don't fall into condiment/bread/temp modification paths.
    {
      ...
      if (_swapSig) {
        ...
        if (_toTextSwap && _toTextSwap.length >= 2 && !_toIsBreadOrFormat) {
          ...
          if (!_swapTarget) {
            const _swapMeatId = inferMeatFromText(_toTextSwap, config.meatIndex); console.error("SWAP_MEAT: toText="+_toTextSwap+" meatId="+_swapMeatId+" sameCatLen="+_sameCatMenu.length);
            if (_swapMeatId) {
              ...
              if (_byMeat.length === 1) _swapTarget = _byMeat[0]; console.error("BY_MEAT len="+_byMeat.length+" ids="+_byMeat.map(x=>x.id).join(","));
              else if (_byMeat.length > 1) _swapTarget = findBestMenuItem(normalizeText(_toTextSwap) + (_origCategory ? " " + _origCategory : ""), _byMeat);
            }
          }
...
async function main() {
  ...
  function systemSay(lines) {
    ...
    for (const line of out) console.log(`SYSTEM: ${String(line)}`);
    if (process.env.NAPOLI_DEBUG_PROMPTS === "1") {
    ...
    while (steps < 15) {
      ...
      if (out.responses.length) {
        ...
        if (state._correction_inline_text && state.phase === "AWAITING_ORDER_CORRECTION") {
          ...
          // If applyPhaseInput triggered a start-over reset, skip rules — the main loop
          // will emit the fresh-start message via the _start_over_complete guard.
          if (!state._start_over_complete) {
    ...
    if (combinedResponses.length) {
      ...
      if (hasOrderSignal && state.intent !== "place_order") {
        ...
        if (outRetry.responses.length) {
          ...
          console.log("SYSTEM: (no rule matched — try rephrasing)");
        }
      } else {
        console.log("SYSTEM: (no rule matched — try rephrasing)");
      }
